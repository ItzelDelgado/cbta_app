...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('inspeccion_mezclas', function (Blueprint $table) {
            ...
            // ÚNICA inspección por mezcla
            $table->foreignId('mezcla_id')
                ->constrained('mezclas')
                ->onDelete('cascade')
                ->unique();

            // Booleans con default (evita tener que mandarlos siempre)
            ...
            // Firmas
            $table->string('reviso_nombre');      // déjalo NOT NULL si así lo quieres
            $table->string('aprobo_nombre');      // idem
