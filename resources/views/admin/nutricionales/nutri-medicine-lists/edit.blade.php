<x-admin-layout>
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Editar lista nutricional</h1>

    <form action="{{ route('admin.nutricionales.nutri-medicine-lists.update', $nutriMedicineList) }}" method="POST"
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $nutriMedicineList->name) }}"
                ...
                <div>
                    ...
                    <select name="is_active" class="w-full rounded border-gray-300">
                        <option value="1"
                            {{ old('is_active', (string) $nutriMedicineList->is_active) == '1' ? 'selected' : '' }}>Sí
                        </option>
                        <option value="0"
                            {{ old('is_active', (string) $nutriMedicineList->is_active) == '0' ? 'selected' : '' }}>No
                        </option>
                    </select>
                </div>
            </div>
            ...
            <div class="mt-4 flex items-center">
                ...
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="active_brands" value="1" class="sr-only peer"
                        {{ old('active_brands', $nutriMedicineList->active_brands) ? 'checked' : '' }}>
                    <div
            ...
            <div class="mt-4">
                ...
                <textarea name="description" rows="3" class="w-full rounded border-gray-300">{{ old('description', $nutriMedicineList->description) }}</textarea>
            </div>

            @php
                $distributor = $nutriMedicineList->distributor;
            @endphp

            <div class="mt-6 border rounded-lg p-4 bg-gray-50">
                <h2 class="text-sm font-semibold text-gray-800">Distribuidor</h2>
                <p class="text-xs text-gray-500 mb-4">
                    Estos datos se usarán para la segunda hoja de remisión nutricional.
                </p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center gap-4">
                        @if (!empty($distributor?->logo_path))
                            <img src="{{ asset('storage/' . $distributor->logo_path) }}"
                                alt="Logo distribuidor" class="h-16 object-contain rounded border bg-white">
                        @endif
                                        ...
                                        <table class="min-w-full border border-gray-200 rounded-lg">
                                            ...
                                            <tbody>
                                                @forelse ($catalog->presentations as $presentation)
                                                    @php
                                                        $item = $itemsByPresentation[$presentation->id] ?? null;
                                                    @endphp
                                                    ...
                                                    <tr class="border-b last:border-b-0 medicine-presentation-row"
                                                        ...
                                                        <td class="px-4 py-3">
                                                            <input type="number" step="0.0001" min="0"
                                                                ...
                                                                value="{{ old('items.' . $rowIndex . '.precio_ml', $item?->precio_ml ?? 0) }}"
                                                                class="w-full rounded border-gray-300" required>
                                                        </td>
                                                    </tr>
