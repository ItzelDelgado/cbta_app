<x-admin-layout>
  ...
  <div class="flex justify-end mb-4">
    <a
      class="text-white bg-azul-prodifem hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5"
      ...
      <tbody>
        @forelse ($presentations as $p)
          ...
          <tr class="bg-white border-b">
            ...
            <td class="px-6 py-4">{{ $catalog->denominacion }}</td>
            <td class="px-6 py-4">
              {{ $p->presentacion }}
              @if($p->contenido_valor && $p->contenido_unidad)
                — {{ rtrim(rtrim(number_format($p->contenido_valor, 2, '.', ''), '0'), '.') }} {{ $p->contenido_unidad }}
              @endif
            </td>
            ...
            <td class="px-6 py-4">
              <div class="flex space-x-2">
                <a
                  href="{{ route('admin.oncologicos.medicines.catalog.presentations.edit', [$catalog->id, $p->id]) }}"
