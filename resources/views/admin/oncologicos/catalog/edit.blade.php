<x-admin-layout>
  <div class="mt-2">
    <h1 class="text-2xl font-medium text-gray-800">Editar medicamento (Catálogo)</h1>
  </div>

  <form action="{{ route('admin.oncologicos.medicines.catalog.update', $medicamento->id) }}" method="POST" class="mb-6">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-4 gap-4">
      <div>
        ...
        <input name="denominacion"
               value="{{ old('denominacion', $medicamento->denominacion) }}"
               required
      ...
      <div>
        ...
        <input type="number" step="0.01" min="0"
               name="conc_min"
               value="{{ old('conc_min', $medicamento->conc_min) }}"
               class="w-full p-2 text-sm bg-gray-50 border rounded-lg" />
      </div>
      ...
      <div>
        ...
        <input type="number" step="0.01" min="0"
               name="conc_max"
               value="{{ old('conc_max', $medicamento->conc_max) }}"
               class="w-full p-2 text-sm bg-gray-50 border rounded-lg" />
      </div>
